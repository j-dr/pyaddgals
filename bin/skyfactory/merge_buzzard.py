...
class buzzard_flat_cat(object):
    ...
    def loop_cats(self, debug=False): 
        ...
        for ifile,filename in enumerate(glob.glob(self.rootdir+'/'+self.obsdir+'*'+self.obsname+'*[0-9].fits')):
            ...
            gout = fio.FITS(self.simname+'_{}'.format(self.obsdir[:-1])+'_gold.fits', 'rw') 
            sout = fio.FITS(self.simname+'_{}'.format(self.obsdir[:-1])+'_shape.fits', 'rw') 
            pout = fio.FITS(self.simname+'_{}'.format(self.obsdir[:-1])+'_pz.fits', 'rw')

            tname = filename.replace(self.obsname, self.truthname).replace(self.obsdir, self.truthdir)
            pzname = filename.replace(self.obsname, self.pzname).replace(self.obsdir, self.pzdir).replace('.fits','.BPZ.fits')
            ...
            if not debug:
                ...
                obs    = fio.FITS(filename)[-1].read(columns=['RA','DEC','EPSILON1','EPSILON2','LSS_FLAG', 'WL_FLAG', 'MAG_G', 'MAG_R', 'MAG_I', 'MAG_Z', 'IVAR_G', 'IVAR_R', 'IVAR_I', 'IVAR_Z', 'SIZE'])

            pz     = fio.FITS(pzname)[-1].read(columns=['MEAN_Z','Z_MC','MODE_Z'])
            ...
            if self.nzcut:
                sflag[obs['MAG_R'] > (22.00718733 + 1.21691296 * pz['MEAN_Z'])] = 0
    ...
    def process_single_file(self, obs, truth, pz, sample, rank, debug=False): 
        ...
        gout = fio.FITS(self.odir+'/'+self.simname+'_{}'.format(self.obsdir[:-1])+'_gold.{}.fits'.format(rank), 'rw') 
        sout = fio.FITS(self.odir+'/'+self.simname+'_{}'.format(self.obsdir[:-1])+'_shape.{}.fits'.format(rank), 'rw') 
        pout = fio.FITS(self.odir+'/'+self.simname+'_{}'.format(self.obsdir[:-1])+'_pz.{}.fits'.format(rank), 'rw')
        ...
        gold['hpix']              = hp.ang2pix(4096, np.pi/2.-np.radians(obs['DEC']),np.radians(obs['RA']), nest=True)
    ...
    def merge_rank_files(self, merge_with_bpz=False):
        ...
        gout = fio.FITS(self.odir+'/'+self.simname+'_{}'.format(self.obsdir[:-1])+'_gold.fits', 'rw')
        sout = fio.FITS(self.odir+'/'+self.simname+'_{}'.format(self.obsdir[:-1])+'_shape.fits', 'rw')
        pout = fio.FITS(self.odir+'/'+self.simname+'_{}'.format(self.obsdir[:-1])+'_pz.fits', 'rw')

        gfiles = glob.glob(self.simname+'_{}'.format(self.obsdir[:-1])+'_gold*[0-9].fits')
        size   = len(gfiles)

        for i in range(size):

            gold   = fio.read(self.simname+'_{}'.format(self.obsdir[:-1])+'_gold.{}.fits'.format(i))
            shape  = fio.read(self.simname+'_{}'.format(self.obsdir[:-1])+'_shape.{}.fits'.format(i))
            photoz = fio.read(self.simname+'_{}'.format(self.obsdir[:-1])+'_pz.{}.fits'.format(i))

            idx = ((gold['mag_g']<99) & (gold['mag_r']<99) & (gold['mag_i']<99) & (gold['mag_z']<99) & 
                   np.isfinite(gold['mag_g']) & np.isfinite(gold['mag_r']) & np.isfinite(gold['mag_i'])
                   & np.isfinite(gold['mag_z']))

            if merge_with_bpz:
                bpz = fio.read(self.simname+'_{}'.format(self.obsdir[:-1])+'_gold.{}.BPZ.fits'.format(i))
