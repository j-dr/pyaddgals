FROM ubuntu:18.04
RUN apt-get update && \
    apt-get install -y autoconf automake gcc gfortran g++ zlib1g-dev make cmake wget make git swig pkg-config python3.7 python3-pip libffi-dev libssl-dev gsl-bin libgsl-dev libblas-dev libfftw3-dev && apt-get clean all
...
ENV GSLI /usr/include/gsl/
ENV GSLL /usr/lib/x86_64-linux-gnu/
